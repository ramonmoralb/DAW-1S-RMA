...
public class EmpleatDAO /*extends OficinaDAO*/{ // extendia para poder usar el metodo oficinaxiste
    public List<Empleat> getEmpleatsRangoEdad(int minima, int maxima) {
        ...
        try {
            ...
            while(rs.next()){
                //SqlLite no soporta fechas  por lo que las fechas se han introducido como TEXT
                //las siguientes dos lineas convierten el string a date y lo almacena en un Localdate que se usara para crear el 
                //empleado, de no hacerlo entra la excepcion parse date stamp
                String contracte = rs.getString("contracte");
                LocalDate contracteFormatoDate = LocalDate.parse(contracte);// se convierte a formato que entiende java de fechas
                ...
                Empleat empleat = new Empleat(
                        ...
                        contracteFormatoDate);    // variable valida para la creación del empleado
                        //rs.getDate("contracte").toLocalDate());
                //empleat.mostrar();// cada iteración del bucle mostrará al empleado instanciado
